...
while($dsql->res_tab_assoc($line))
{
	if((!strcmp($currentPage,$line['path']))||(!strcmp($currentPage, substr($line['path'],1))))
	{
		...
		$temp_menu.='<li class="currentTab"><a href="/'.$line['path'].'">'.$line['title'].'</a></li>';
	}
	else
...
<li><input type="submit" value="Log in"/> '.tr('RememberMe?').'<input type="checkbox" name="cookieAct" id="cookieAct" value="cookieAct"/> <a href="">'.tr('_register?_').'</a></li>
</ul>
</form>';
